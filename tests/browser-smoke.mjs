...
try {
  ...
  assert(await page.locator('#tutorial-modal').evaluate(modal => modal.contains(document.activeElement)),'Tutorial focus must remain trapped in dialog')
  ...
  await page.waitForSelector('#last-move-modal:not([hidden])')
  await page.keyboard.press('Escape')
  assert(await page.locator('#last-move-modal').isVisible(),'Escape must not dismiss the mandatory Last Spark decision')
  await page.reload()
  await page.locator('#continue-button').click()
  await page.waitForSelector('#last-move-modal:not([hidden])')
  ...
  const staleDate = await page.evaluate(() => window.__OMM_TEST__.prepareStaleDaily())
  assert(staleDate === '2000-01-01','Test fixture must create a stale Daily')
  await page.locator('#continue-button').click()
